...
def crawler():
    ...
    for cat in gig_category[0:1]:
        ...
        for a in data:
            ...
            filename = "negative_"+(cat.text).strip("\n\t ") + "/" +(a.text).strip("\n\t ")
            ...
            for link in soup.find_all('a', {'class': 'gig-link-main'}):
                ...
                soup = BeautifulSoup(browser.page_source)
                try:
                    browser.find_element_by_link_text("MOST RECENT").click();
                    browser.find_element_by_link_text("NEGATIVE REVIEWS").click();
                    time.sleep(10);
                except:
                    continue
                revCountInit=""
                ...
                for review_count in soup.find_all('h4',{'itemprop':'reviewCount'}):
                    s += ("\"Total Number of Reviews\":\""+(review_count.text).strip("\r\n\t").replace("\"","\\\"")+"\",\n\t").encode('utf-8')
                data2 = soup.find_all('ul', {'class': 'reviews-list'})
                
                count_review = 1
                for ul2 in data2:
                    s+=("\"Negative Reviews\":\t[ "). encode('utf-8')
                    all_reviews = ul2.find_all('li',{'class':'star-rating-row'})
                    only_feedback = ul2.find_all('li',{'class':'rating-seller'})
                    required_array = [x for x in all_reviews if x not in only_feedback]
                    for li2 in required_array:
                        ...
                        for a2 in linkss:
                            ...
                            count_r += 1
                        time.sleep(1)
                        sender_feedback = li2.findNext('li')
                        s+=("\"Sender Feedback\":["). encode('utf-8')
                        while(sender_feedback):
                          if ('rating-seller' in sender_feedback['class']):
                            sender_msg = sender_feedback.find('div',{'class':'msg-body'})
                            s+=("\n\t\t\t{\n\t\t\t\t\"Feedback\":\""+(sender_msg.text).strip("\r\n\t").replace("\"","\\\"")+"\"\n\t\t\t},").encode('utf-8')
                            sender_feedback = sender_feedback.findNext('li');
                          else:
                            s+=("\n\t\t\t{\n\t\t\t\t\"Feedback\":\""+("").strip("\r\n\t").replace("\"","\\\"")+"\"\n\t\t\t}").encode('utf-8')
                            sender_feedback = 0;
                        s+=("\n]},");
